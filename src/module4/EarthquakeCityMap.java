...
public class EarthquakeCityMap extends PApplet {
	...
	private void printQuakes() 
	{
		...
		HashMap<String, Integer> quackes = new HashMap<String, Integer>();
		int landCount = 0;
		
		for (Marker countryMarker : countryMarkers) {
			String placeName = (String)countryMarker.getProperty("name");
			for (Marker quackeMarker : quakeMarkers) {
				String land = (String)quackeMarker.getProperty("country");
				EarthquakeMarker em = (EarthquakeMarker)quackeMarker;
				if(em.isOnLand() && placeName.equals(land)) {
					if (quackes.containsKey(placeName)) {
						quackes.put(placeName, quackes.get(placeName) + 1);
					}
					else {
						quackes.put(placeName, 1);
					}
					landCount++;
				}
			}
		}
		quackes.put("OCEAN QUAKES", quakeMarkers.size() - landCount);
		System.out.println(quackes);
	}
